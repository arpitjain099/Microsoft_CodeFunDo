...
namespace Microsoft.Samples.Kinect.Avateering
{
    ...
    public class AvateeringXNA : Microsoft.Xna.Framework.Game
    {
        ...
        private const int WindowedHeight = 600;
        ...
        #region OurVariableDeclaration

        ///////////////// OUR CODE BEGINS
        private int level;
        private int ballPos;
        Texture2D buttonStart, buttonOptions, buttonResume, buttonExit;
        Texture2D optionBall;
        private int leftHandSelect;
        private int rightHandSelect;
        private int rightHandMenu;
        //there are 4 types of ball, this one is the type of the currently falling ball
        private int fallingBallType;
        //this integer has the value of the width from the left of the falling ball
        private int fallingBallWidth;
        // CUrrent X location of the falling ball
        private int fallingBallX;
        //this one has the value of the height from the top of the falling ball
        private int fallingBallHeight;
        // Current Y location of the falling ball
        private int fallingBallY;
        //this one tells us whether we need to add another falling ball or not
        private bool addFallingBall;
        //these are the 4 types of balls
        Texture2D firstBall, secondBall, thirdBall, fourthBall;
        //this list contains the type of all the balls lying on floor
        List<int> lyingBalls = new List<int>();
        //this list has the X coordinate of all the balls lying on the floor
        List<int> XBalls = new List<int>();
        //this list has the Y coordinate of all the balls lying on the floor, probably same 
        List<int> YBalls = new List<int>();
        //this is an array of size 5, 
        //element[0]=null
        //element[1]=firstBall (firstBall.png)
        //element[2]=secondBall ...
        //element[3]=thirdBall ...
        //element[4]=fourthBall ...
        List<Texture2D> typePic = new List<Texture2D>();
        // 2-D array storing the balls
        int[][] ballStore;
        ...
        // Screen Options 
        Texture2D screenBg;
        int ScreenXL, ScreenXR, ScreenYT, ScreenYB;
        int ScreenBallWidth, ScreenBallHeight;
        int ScreenXLimit, ScreenYLimit;
        private readonly SoundPlayer squeezeSound;
        private int timer;
        private int speed;
        private int points;
        ///////////////// OUR CODE ENDS
        ...
        public AvateeringXNA()
        {
            ...
            this.level = 1;
            this.rightHandMenu = 0;
            this.leftHandSelect = 0;
            this.ballPos = 0;
            this.difficulty = 4;
            this.fallingBallWidth = 30;
            this.fallingBallHeight = 30;
            this.ScreenBallWidth = 400;
            this.ScreenBallHeight = 500;
            this.random = new Random();
            this.ScreenXLimit = ScreenBallWidth / fallingBallWidth;
            this.ScreenYLimit = ScreenBallHeight / fallingBallHeight - 2;
            this.ScreenXL = WindowedWidth / 2 - ScreenBallWidth / 2;
            this.ScreenXR = WindowedWidth / 2 + ScreenBallWidth / 2;
            this.ScreenYB = 20+ ScreenBallHeight;
            this.ScreenYT = 20;
            this.timer = 0;
            this.speed = 30;
            this.points = 0;
             ballStore = new int[ScreenYLimit][];
             for (int i = 0; i < ScreenYLimit; i++)
                 ballStore[i] = new int[ScreenXLimit];
             squeezeSound = new System.Media.SoundPlayer(@"H:\WIN APP FEST\New folder\bgmusic.wav");
        }
        ...
        protected override void LoadContent()
        {
            ...
            buttonResume = Content.Load<Texture2D>("resume");
            buttonOptions = Content.Load<Texture2D>("options");
            optionBall = Content.Load<Texture2D>("optionBall");
            firstBall = Content.Load<Texture2D>("firstBall");
            secondBall = Content.Load<Texture2D>("secondBall");
            thirdBall = Content.Load<Texture2D>("thirdBall");
            fourthBall = Content.Load<Texture2D>("fourthBall");
            screenBg = Content.Load<Texture2D>("Screen");

            //********************************************************************

            this.addFallingBall = true;
            this.fallingBallType = 0;

            typePic.Add(optionBall);
            typePic.Add(firstBall);
            typePic.Add(secondBall);
            typePic.Add(thirdBall);
            typePic.Add(fourthBall);
                ...
                if (null != skeletonFrame)
                {
                    ...
                    if (null != this.animator)
                    {
                        if (null != rawSkeleton)
                        {
                            ...
                            /////////////// Our Code
                            float handLX = rawSkeleton.Joints[JointType.HandLeft].Position.X;
                            float handLY = rawSkeleton.Joints[JointType.HandLeft].Position.Y;
                            float handRX = rawSkeleton.Joints[JointType.HandRight].Position.X;
                            float handRY = rawSkeleton.Joints[JointType.HandRight].Position.Y;
                            float headY = rawSkeleton.Joints[JointType.Head].Position.Y;
                            float headX = rawSkeleton.Joints[JointType.Head].Position.X;
                            float shoulRY = rawSkeleton.Joints[JointType.ShoulderRight].Position.Y;
                            float shoulRX = rawSkeleton.Joints[JointType.ShoulderRight].Position.X;
                            float shoulLY = rawSkeleton.Joints[JointType.ShoulderLeft].Position.Y;
                            float shoulLX = rawSkeleton.Joints[JointType.ShoulderLeft].Position.X;
                            float hipX = rawSkeleton.Joints[JointType.HipCenter].Position.X;
                            float hipY = rawSkeleton.Joints[JointType.HipCenter].Position.Y;

                            float distRightX = shoulRX - headX;
                            float distRightY = headY - shoulRY;

                            float distLeftX = headX - shoulLX;
                            float distLeftY = headY - shoulLY;

                            ///////////// Menu Commands
                            this.rightHandMenu = 0;
                            if (handRX - headX > 2 * distRightX)
                            {
                                if (handRY < headY && handRY > (shoulRY + hipY)/2) this.rightHandMenu = 2;
                                else if (handRY > headY)                this.rightHandMenu = 1;
                                else if (handRY < (shoulRY + hipY) / 2) this.rightHandMenu = 3;
                            }

                            this.leftHandSelect = 0;
                            if (headX - handLX > 2 * distLeftX)
                            {
                                this.leftHandSelect = 1;
                            }

                            if (this.leftHandSelect == 1 && this.rightHandMenu == 1 && level == 0)
                                level = 1;

                            ///////////////
                            /////////////////// Ball commands
                            
                            this.rightHandSelect = 0;
                            if (handRX -headX> 2 * distRightX)
                            {
                                this.rightHandSelect = 1;
                            }
                         //   Console.WriteLine("Check " + fallingBallType + " " + fallingBallX + " " + fallingBallY);      
                            if (this.fallingBallType == 0 && this.addFallingBall==true)
                            {
                                //there is no falling ball in the picture and we want one
                                //write these 3 functions
                                //getBallType() will return what type of ball should come this time
                                //getWidth() returns where it should be placed on width axis
                                //getHeight() returns where it should be placed on top, (at the top probably)
                               
                                //this.fallingBallType = getBallType();
                                this.fallingBallType = random.Next(1,difficulty+1);
                                this.fallingBallX = ScreenXLimit/2;
                                this.fallingBallY = 1;
                                Console.WriteLine("Assign " + fallingBallType + " " + fallingBallX + " " + fallingBallY);               
                                //this.fallingBallWidth = getWidth();
                                //this.fallingBallHeight = getHeight();
                                //********************************************************************
                                
                                //we now have a falling ball which will be drawn on the next one
                                this.addFallingBall = false;
                                //no need to have to add another falling ball
                            }
                            else if (0 == this.fallingBallType)
                            {
                                //this means that we dont have a falling ball, we are doing the logic
                                //for removing (if we can remove some balls) balls in vicinity
                                //write the logic here



                                //after removing all the balls we can, we will now handle a new falling ball
                                this.addFallingBall = true;
                            }
                            else
                            {
                                //we already have a falling ball; using hand gestures to move it
                                if (this.leftHandSelect == this.rightHandSelect)
                                {
                                    //moveDown() returns how much down the fall will go, it is bounded by end of the screen
                                    moveDown();
                                    //don't do anything to move it left or right
                                    //both values in the conditional above can be 0 or 1.
                                }
                                else if (this.leftHandSelect == 1)
                                {
                                    //moving the ball left
                                    //moveLeft() return how much left the ball will move, it is upper bounded by the leftmost end
                                    moveLeft();
                                    //moveDown() returns how much down the fall will go, it is bounded by end of the screen
                                    moveDown();

                                }
                                if (this.rightHandSelect == 1)
                                {
                                    //moveRight() return how much right the ball will move, it is upper bounded by the rightmost end
                                    moveRight();
                                    //moveDown() returns how much down the fall will go, it is bounded by end of the screen
                                    moveDown();
                                }

                                //if the falling ball has reached the end
                                //getEnd() return the end place where to a ball can fall
                                if (this.fallingBallY == getEnd())
                                {
                                    //lyingBalls.Add(fallingBallType);
                                    //XBalls.Add(fallingBallX);
                                    //YBalls.Add(fallingBallY);

                                    //if (fallingBallX == XBalls[fallingBallType])
                                    //check this line
                                    Console.WriteLine("FALLING "+fallingBallX+" "+ ((fallingBallType - 1) * (ScreenXLimit - 1) / 3) +" "+ (fallingBallWidth)+" " + ScreenXL);

                                    if (fallingBallX == (((fallingBallType - 1) * (ScreenXLimit-1)) / 3))
                                    //   if(true)
                                        this.points+=this.difficulty;
                                    this.fallingBallType = 0;
                                    //this.ballStore[fallingBallY][fallingBallX] = fallingBallType;
                                }
                            }
                            //********************************************************************
        ...
        {
            if (fallingBallX > 0)
                fallingBallX = fallingBallX - 1 ;
        }
        void moveRight()
        {
            if (fallingBallX < ScreenXLimit-1)
                fallingBallX = fallingBallX + 1;
        }
        void moveDown()
        {
            if (fallingBallY < getEnd())
                fallingBallY= fallingBallY + 1;
        }
        int getEnd()
        {
            int i;
            for (i = ScreenYLimit-1; i >=0; i--)
            {
                if (ballStore[i][fallingBallX] != 1) return i;
            }
            return i;
        }
        /// <summary>
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {   //menu page of the screen
                ...
                spriteBatch.Draw(buttonStart, new Rectangle(WindowedWidth/2-130,WindowedHeight/2 - 45-110, 260, 90), Color.White);
                spriteBatch.Draw(buttonOptions, new Rectangle(WindowedWidth/2 - 130, WindowedHeight/2 - 45, 260, 90), Color.White);
                spriteBatch.Draw(buttonExit, new Rectangle(WindowedWidth/2 - 130, WindowedHeight/2 - 45 + 110, 260, 90), Color.White);
                /*
                spriteBatch.Draw(buttonStart, new Rectangle(350, 100, 150, 50), Color.White);
                //spriteBatch.Draw(buttonResume, new Rectangle(100, 200, 150, 50), Color.White);
                spriteBatch.Draw(buttonOptions, new Rectangle(350, 300, 150, 50), Color.White);
                spriteBatch.Draw(buttonExit, new Rectangle(350, 400, 150, 50), Color.White);
                */
                //now we need to draw a ball and use gestures to select the options
                if (rightHandMenu > 0)
                {
                    spriteBatch.Draw(optionBall, new Rectangle(WindowedWidth/2 + 130, WindowedHeight/2- 260 + 110 * rightHandMenu, 60, 60), Color.White);
                }
                /*
                if(ballPos == 0)
                {
                    spriteBatch.Draw(optionBall, new Rectangle(200, 90, 100, 100), Color.White);
                }
                else if (ballPos == 1)
                {
                    spriteBatch.Draw(optionBall, new Rectangle(200, 190, 100, 100), Color.White);
                }
                else if (ballPos == 2)
                {
                    spriteBatch.Draw(optionBall, new Rectangle(200, 290, 100, 100), Color.White);
                }
                else if (ballPos == 3)
                {
                    spriteBatch.Draw(optionBall, new Rectangle(200, 390, 100, 100), Color.White);
                }
                else
                {
                    //if anyhow we get an invalid code, move it back to 0
                    ballPos = 0;
                }
                */
                spriteBatch.End();
            ...
            {
                //this is when we are in the game
                ...
                */
                ...
                //********************************************************************
                spriteBatch.Begin();
                spriteBatch.Draw(screenBg, new Rectangle(WindowedWidth / 2 - ScreenBallWidth/2, 20 , ScreenBallWidth, ScreenBallHeight), Color.White);
         //       spriteBatch.Draw(buttonStart, new Rectangle(WindowedWidth / 2 - 130, WindowedHeight / 2 - 45 - 110, 260, 90), Color.White);
                //spriteBatch.Draw(firstBall, new Rectangle(2 * fallingBallWidth + ScreenXL, 4 * fallingBallHeight + ScreenYT, fallingBallWidth, fallingBallHeight), Color.White);
                //Console.WriteLine("Print Ball " + (2 * fallingBallWidth + ScreenXL) + " " + (4 * fallingBallHeight + ScreenYT) + " " + fallingBallWidth + " " + fallingBallHeight);
                //drawing balls
                
                /*for (int counter = 0;counter< YBalls.Count;counter++)
                {
                    //drawing all the lying balls
                    spriteBatch.Draw(typePic[lyingBalls[counter]], new Rectangle(XBalls[counter] * fallingBallWidth + ScreenXL, YBalls[counter] * fallingBallHeight + ScreenYT, fallingBallWidth, fallingBallHeight), Color.White);
                    //Console.WriteLine("Lying ball "+YBalls[counter]+" "+ XBalls[counter] +" "+ fallingBallWidth+" " + ScreenXL);
                }*/

                //Console.WriteLine("PROBLEMS " + fallingBallType+" "+ addFallingBall+ " "+fallingBallX+ " "+ fallingBallY );
                if (this.fallingBallType > 0)
                {
                    // you have a falling ball, so based on the type draw it
                    spriteBatch.Draw(typePic[fallingBallType], new Rectangle(fallingBallX * fallingBallWidth + ScreenXL, fallingBallY * fallingBallHeight + ScreenYT, fallingBallWidth, fallingBallHeight), Color.White);
                   // Console.WriteLine("Falling Ball "+ fallingBallY+" "+fallingBallX +" "+ fallingBallWidth +" "+ ScreenXL);
                }

        /*       if(leftHandSelect==1)
                spriteBatch.Draw(typePic[1], new Rectangle(-fallingBallWidth + ScreenXL, fallingBallHeight + ScreenYT, fallingBallWidth, fallingBallHeight), Color.White);
               if (rightHandSelect==1)
                   spriteBatch.Draw(typePic[1], new Rectangle(fallingBallWidth + ScreenXR, fallingBallHeight + ScreenYT, fallingBallWidth, fallingBallHeight), Color.White);
          */
                spriteBatch.DrawString(Content.Load<SpriteFont>("SpriteFont1"), "Score", new Vector2(ScreenXL-100, 100), Color.Black);
                spriteBatch.DrawString(Content.Load<SpriteFont>("SpriteFont1"), ""+points, new Vector2(ScreenXL - 100, 150), Color.Black);
               
                //printing the holes
                for (int i = 1; i <= 4; i++)
                {
                    spriteBatch.Draw(typePic[i], new Rectangle((((i-1)*(ScreenXLimit-1))/3)*fallingBallWidth + ScreenXL, ScreenYLimit*fallingBallHeight + ScreenYT, fallingBallWidth, fallingBallHeight), Color.White);
